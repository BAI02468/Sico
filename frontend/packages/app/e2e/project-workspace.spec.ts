...
test.describe("project workspace", () => {
  ...
  test(
    "renders the project name and assets toolbar once loaded",
    { tag: ["@core", "@project"] },
    async ({ page }) => {
  ...
  test(
    "a direct page load of /project/$id shows the full-page skeleton until the project resolves",
    { tag: ["@loading", "@project"] },
    async ({ page }) => {
  ...
  test(
    "a direct page load of a category sub-route (/knowledge) shows the same full-page skeleton",
    { tag: ["@loading", "@project"] },
    async ({ page }) => {
  ...
  test(
    "a direct page load shows the in-table skeleton once the shell is up but rows are still loading",
    { tag: ["@loading", "@project"] },
    async ({ page }) => {
  ...
  test(
    "renders the full-page error view with Try again when project detail 500s",
    { tag: ["@error", "@project"] },
    async ({ page }) => {
      await mockEndpoint(page, "knowledge/tags", () => ({
        body: makeOkEnvelope({ tags: [], total: 0 }),
  ...
  test(
    "renders the assets empty state when the category list is empty",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
  ...
  test(
    "renders an in-table error view when the assets list 500s",
    { tag: ["@error", "@project"] },
    async ({ page }) => {
  ...
  test(
    "Add Knowledge: a link import surfaces the success toast",
    { tag: ["@key", "@knowledge"] },
    async ({ page }) => {
      await mockWorkspaceSuccess(page);
      ...
      await expect(
        dialog.getByText("https://example.com/doc.pdf"),
      ).toBeVisible();
  ...
  test(
    "navigating to a knowledge row opens its detail page",
    { tag: ["@core", "@project"] },
    async ({ page }) => {
      await mockWorkspaceSuccess(page, {
  ...
  test(
    "editing a knowledge row from its actions menu saves and toasts",
    { tag: ["@key", "@knowledge"] },
    async ({ page }) => {
      await mockWorkspaceSuccess(page, {
        documents: [makeKnowledgeDocument(7, { name: "Onboarding Guide" })],
      });
      // PUT /knowledge/document returns a bare ok envelope; the row GET keeps
      // serving the document so the dialog seeds its fields.
      await mockEndpoint(page, "knowledge/document", () => ({
        body: makeOkEnvelope({}),
      }));

      await page.goto(OVERVIEW_URL);
      // Open the row's "…" actions menu → Edit. The Add-Knowledge success test
      // proves the menu renders under project_admin (granted by mockWorkspaceSuccess).
      await page.getByRole("button", { name: "Asset actions" }).first().click();
      await page.getByRole("menuitem", { name: "Edit" }).click();

      const dialog = page.getByRole("dialog", { name: "Edit" });
      await expect(dialog).toBeVisible();
      await dialog.getByLabel("Knowledge name").fill("Onboarding Guide v2");
  ...
  test(
    "the Experience tab is a route link that loads only playbook rows",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
  ...
  test(
    "the Deliverable tab is a route link that loads only deliverable rows",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
      await mockWorkspaceSuccess(page, {
        documents: [makeKnowledgeDocument(1, { name: "A Knowledge Doc" })],
  ...
  test(
    "a direct page load of /experience selects the Experience tab and loads its rows",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
  ...
  test(
    "searching filters rows and syncs ?q to the URL",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
      await mockWorkspaceSuccess(page, {
        documents: [
  ...
  test(
    "toggling the CREATED TIME header flips the sort and syncs ?sort",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
      await mockWorkspaceSuccess(page, {
  ...
  test(
    "collapsing the drawer hides it and the restore button brings it back",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
      await mockWorkspaceSuccess(page);
  ...
  test(
    "editing the project from the drawer toasts and closes on success",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
  ...
  test(
    "deleting the project from the drawer menu confirms and toasts",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
      await mockWorkspaceSuccess(page, { project: { name: "Acme" } });
      // DELETE /project?id returns a bare ok envelope (assertOk-only).
      await mockEndpoint(page, "project", (url) =>
        url.searchParams.has("id")
          ? {
              body: makeOkEnvelope({
      ...
      await page.goto(OVERVIEW_URL);
      // The delete action lives behind the drawer's "…" overflow menu (admin-only).
  ...
  test(
    "a viewer with no project role sees no manage affordances",
    { tag: ["@key", "@project"] },
    async ({ page }) => {
      await mockWorkspaceSuccess(page, { project: { name: "Acme" } });
      // Override the fixture's admin grant with an empty roster (most-recent route
      // wins): the viewer holds no project role, so every manage-gated affordance
      // — Edit project, the "…" actions menu, and Add Knowledge — must be absent.
      await mockEndpoint(page, "rbac/user_roles", () => ({
        body: makeOkEnvelope({ roles: [], total: 0 }),
      }));

      await page.goto(OVERVIEW_URL);
      // The workspace still loads (name + tabs render for everyone)…
      await expect(
        page.getByRole("heading", { level: 1, name: "Acme" }),
      ).toBeVisible();
      // …but no manage affordances are exposed.
      await expect(
  ...
  test(
    "a knowledge row whose extraction failed shows the failure state",
    { tag: ["@error", "@knowledge"] },
    async ({ page }) => {
      // status 1 = FAILED extraction. The row swaps its "Knowledge" TYPE label for
      // a red "Extraction failed" unit instead of navigating.
      await mockWorkspaceSuccess(page, {
        documents: [
          makeKnowledgeDocument(7, { name: "Broken Upload", status: 1 }),
        ],
      });

      await page.goto(OVERVIEW_URL);
      await expect(page.getByText("Broken Upload")).toBeVisible();
      await expect(page.getByText("Extraction failed")).toBeVisible();
    },
  );

  test(
    "a knowledge row still extracting shows a shimmer, not a nav link",
    { tag: ["@loading", "@knowledge"] },
    async ({ page }) => {
      // status 2 = UPLOADED (extracting). The name renders as a non-interactive
      // shimmer — the row isn't yet navigable.
      await mockWorkspaceSuccess(page, {
        documents: [
          makeKnowledgeDocument(7, { name: "Ingesting Doc", status: 2 }),
        ],
      });

      await page.goto(OVERVIEW_URL);
      await expect(page.getByText("Ingesting Doc")).toBeVisible();
      // Extracting rows are not navigable buttons.
      await expect(
        page.getByRole("button", { name: /Ingesting Doc/ }),
      ).toHaveCount(0);
    },
  );
});

// REAL environment (@real): the read-only /project/80 workspace shell on live
// data. No mocking — runs only when `SICO_E2E_URL` is set. The one locked
// literal is the title "Sico Demo", an owner-guaranteed stable fixture project;
// everything else is structural so it survives real-data drift.
test.describe("project workspace @real", () => {
  const READONLY_PROJECT = "/project/80";
  const READONLY_PROJECT_TITLE = "Sico Demo";

  test(
    "read-only /project/80 renders its real assets shell",
    { tag: ["@core", "@project"] },
    async ({ page }) => {
      skipWithoutCreds("admin");
      await realLogin(page, "admin");

      await page.goto(READONLY_PROJECT, { waitUntil: "networkidle" });
      // Stable project title (owner-guaranteed read-only fixture).
      await expect(
        page.getByRole("heading", { name: READONLY_PROJECT_TITLE }),
      ).toBeVisible();
      // The assets toolbar affordance is present regardless of asset count.
      await expect(
        page.getByRole("button", { name: "Add Knowledge" }),
      ).toBeVisible();
  ...
  test(
    "real: opening the first asset row navigates to its detail page",
    { tag: ["@core", "@project"] },
    async ({ page }) => {
      // Read-only navigation twin: a navigable asset row's name is a <button>
      // that opens the asset detail route. Click the first data row's name and
      // assert we land on a knowledge/experience/deliverable detail URL under
      // /project/80. Drift-proof (any numeric id); skips if the project is empty.
      skipWithoutCreds("admin");
      await realLogin(page, "admin");
      await page.goto(READONLY_PROJECT, { waitUntil: "networkidle" });

      // Scope to data rows (they hold `cell`s; the header row has
      // `columnheader`s), then the first row's name button.
      ...
      test.skip(
        ...
        "No assets on the real /project/80 to open",
      );
      await firstDataRow.getByRole("button").first().click();
  ...
  // RBAC end-to-end on live data: the SAME /project/80, viewed by two real
  // accounts with different project roles, gates its manage affordances
  // differently — the one thing unit tests (which mock the role) can't prove.
  // admin (jialiangbai) is project_admin; user (v-xingkezhu) is project_member.
  // Maps to capabilities.ts: admin has canManageProject (Edit project + Project
  // actions); a member lacks it but keeps canManageAssetOwn (Add Knowledge).

  test(
    "real admin sees the project-manage affordances on /project/80",
    { tag: ["@core", "@project"] },
    async ({ page }) => {
      skipWithoutCreds("admin");
      await realLogin(page, "admin");
      ...
      // project_admin → canManageProject: both manage entry points render.
      await expect(
        page.getByRole("button", { name: "Edit project" }),
      ).toBeVisible();
      await expect(
  ...
  test(
    "real member is denied project-manage but keeps own-asset rights",
    { tag: ["@core", "@project"] },
    async ({ page }) => {
      skipWithoutCreds("user");
      await realLogin(page, "user");
      await page.goto(READONLY_PROJECT, { waitUntil: "networkidle" });

      // The workspace still loads for a member…
      await expect(
        page.getByRole("heading", { name: READONLY_PROJECT_TITLE }),
      ).toBeVisible();
      // …but project_member lacks canManageProject → no Edit / Project actions.
      await expect(
        page.getByRole("button", { name: "Edit project" }),
      ).toHaveCount(0);
      await expect(
      ...
      // canManageAssetOwn is retained → Add Knowledge stays available.
      await expect(
        page.getByRole("button", { name: "Add Knowledge" }),
      ).toBeVisible();
      await expect(page.getByRole("alert")).toHaveCount(0);
