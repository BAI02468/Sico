...
// (`queryClient`, `api`, `store`). A circular import with `./services/api`
// is safe because `api.ts` only dereferences `router` inside a closure
// (`onUnauthorized`), so by the time that closure runs `router` is
// fully initialised.
export const router = createRouter({
  routeTree,
  context: { queryClient, apiClient: api, store },
});
