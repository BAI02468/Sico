...
describe("globals.css", () => {
  ...
  it("maps the subtle Button foreground through secondary semantics", () => {
    expect(css).toMatch(
      /--color-button-subtle-foreground-rest:\s*var\(--color-foreground-secondary\);/,
    );
    expect(css).toMatch(
      /--color-foreground-secondary:\s*var\(\s*--color-neutral-800\s*\);/,
    );
  });
