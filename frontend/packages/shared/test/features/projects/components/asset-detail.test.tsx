...
const PROJECT_ID = 7;
const ASSET_ID = 34;
const ZH_DETAIL_MESSAGES = {
  "projects.assetDetailLayout.detail": "详情",
};
...
function seedProject(queryClient: QueryClient): void {
  queryClient.setQueryData(projectKeys.detail(PROJECT_ID), {
    id: PROJECT_ID,
    name: PROJECT_NAME,
    description: "",
    iconUrl: "",
...
describe("<AssetDetail>", () => {
  ...
  it("updates the Detail panel label when the active locale changes", async () => {
    renderDetail(experience());
    ...
    act(() => {
      i18n.loadAndActivate({ locale: "zh-CN", messages: ZH_DETAIL_MESSAGES });
    });

    expect(
      screen.getByRole("heading", { level: 2, name: "详情" }),
    ).toBeInTheDocument();
    expect(screen.getByRole("region", { name: "详情" })).toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { level: 2, name: "Detail" }),
