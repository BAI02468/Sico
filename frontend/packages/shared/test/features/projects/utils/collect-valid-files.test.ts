...
describe("collectValidFiles", () => {
  it("accepts supported types under the size + count limits", () => {
    const { accepted, rejections } = collectValidFiles(
  ...
  it("rejects an unsupported extension with the wrong-type reason", () => {
    const { accepted, rejections } = collectValidFiles([file("notes.txt")], []);

    expect(accepted).toEqual([]);
    expect(rejections).toEqual([
      { reason: "unsupportedType", filename: "notes.txt" },
    ]);
  });

  it("rejects a file larger than 10MB with the oversize reason", () => {
    const { accepted, rejections } = collectValidFiles(
    ...
    expect(accepted).toEqual([]);
    expect(rejections).toEqual([{ reason: "tooLarge", filename: "big.pdf" }]);
  });
  ...
  it("rejects files past the 5-file cap, counting already-added files", () => {
    ...
    // count gate (one rejection per excess file).
    expect(accepted.map((f) => f.name)).toEqual(["d.pdf"]);
    expect(rejections).toEqual([{ reason: "tooMany" }, { reason: "tooMany" }]);
  });
