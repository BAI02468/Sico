...
describe("useSidebarCollapsed", () => {
  ...
  it("is true while only the Sidepane force-collapse is set (persisted pref expanded)", () => {
    // Regression: the Notification nav row must read the EFFECTIVE state, so a
    // Sidepane-forced collapse switches it to the rail (icon-only) variant.
    // Reading the persisted atom alone would leave it expanded while the Sidebar
    // itself shows the collapsed rail.
    const store = createStore();
