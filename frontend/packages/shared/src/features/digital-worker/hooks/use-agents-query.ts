...
export function selectAgentsRefetchInterval(
  data: InfiniteData<Paged<Agent>> | undefined,
): number {
  const agents = data?.pages.flatMap((page) => page.items) ?? [];
  return conversationStatusRefetchInterval(agents);
}
...
export function agentsQueryOptions(
  ...
  return {
    ...
    staleTime: 30_000,
    refetchInterval: ({ state: { data } }) => selectAgentsRefetchInterval(data),
...
export function useAgentsQuery(
  ...
  return useInfiniteQuery({
    ...agentsQueryOptions(params, apiClient),
...
export function useAgentQuery(
  ...
  const apiClient = useApiClient();
  return useQuery({
    ...agentQueryOptions(agentId, apiClient),
    enabled: options?.enabled,
  });
}
