...
export function Deliverable({
  ...
  // Hook `t` (subscribed) so the preview label re-resolves on a locale switch;
  // the util stays a pure transform and takes the finished string.
  const renderable = toRenderableDeliverables(
    deliverables,
    t({ id: "chat.deliverable.previewPage", message: "Preview Page" }),
  );
