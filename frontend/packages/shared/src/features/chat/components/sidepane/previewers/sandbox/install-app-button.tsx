...
export function InstallAppButton({
  ...
  const onFileChange = (event: ChangeEvent<HTMLInputElement>): void => {
    ...
    if (!file.name.toLowerCase().endsWith(".apk")) {
      toast.error(
        t({
          id: "chat.installAppButton.invalidApk",
    ...
    <>
      {multiple ? (
        <DropdownMenu>
          ...
          >
            {t({
              id: "chat.installAppButton.installApp",
              message: "Install app",
            })}
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => pickFile("current")}>
              {t({
                id: "chat.installAppButton.toThisDevice",
                message: "To this device",
              })}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => pickFile("all")}>
              {t({
                id: "chat.installAppButton.toAllAndroidDevices",
                message: `To all Android devices (${deviceCount})`,
              })}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
