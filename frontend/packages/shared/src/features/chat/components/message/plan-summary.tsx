...
export function PlanSummary({ planId }: PlanSummaryProps): JSX.Element | null {
  ...
  const cards = useMemo(() => {
    ...
    // `t` in the deps so the preview label re-resolves on a locale switch; the
    // util stays a pure transform and takes the finished string.
    return toRenderableDeliverables(
      deliverables,
