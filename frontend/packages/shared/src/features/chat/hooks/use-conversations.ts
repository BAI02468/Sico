...
type Options = UseSuspenseInfiniteQueryOptions<
  ...
  ReturnType<typeof chatKeys.conversationList>,
  number
>;

export function selectConversationsRefetchInterval(
  data: InfiniteData<ConversationListPage> | undefined,
): number {
  const conversations = data?.pages.flatMap((page) => page.items) ?? [];
  return conversationStatusRefetchInterval(conversations);
}
...
export function conversationListQueryOptions(
  ...
  return {
    ...
    refetchInterval: ({ state: { data } }) =>
      selectConversationsRefetchInterval(data),
