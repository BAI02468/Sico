...
export async function createConversation(
  ...
  const res = await apiClient.post<unknown>(CONVERSATION_ENDPOINTS.root, body);
...
export async function getConversation(
  ...
  const res = await apiClient.get<unknown>(CONVERSATION_ENDPOINTS.root, {
    params: { id },
  });
...
export async function listConversations(
  ...
  const res = await apiClient.get<unknown>(CONVERSATION_ENDPOINTS.list, {
    params: {
      agentInstanceId,
      page,
      pageSize,
